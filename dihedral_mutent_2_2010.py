...
def fast_cov(m):
   ...
   weave.inline(code, ['X', 'cov_mat', 'nx', 'ny'],
                type_converters = converters.blitz, compiler = mycompiler, runtime_library_dirs=["/usr/lib64/"], library_dirs=["/usr/lib64/"], libraries=["libstdc++"] )
   return cov_mat
...
def vfast_cov(m):
   ...
   weave.inline(code, ['X', 'cov_mat', 'nx', 'ny'],
                ...
                compiler = mycompiler,runtime_library_dirs="/usr/lib64/", library_dirs=["/usr/lib64/"], libraries=["libstdc++"])
   return cov_mat
...
def sumstuff(ni_vector,numangles_bootstrap,permutations):
    ...
    if(sumstuff_lookup == None):
        ...
        weave.inline(code_create_lookup,['mysum','bootstrap_sets','permutations','ni_vector','nbins','numangles_bootstrap','maxnumangles','sumstuff_lookup'], compiler = mycompiler,runtime_library_dirs=["/usr/lib64/"], library_dirs=["/usr/lib64/"], libraries=["libstdc++"])


    weave.inline(code_do_lookup,['mysum','bootstrap_sets','permutations','ni_vector','nbins','numangles_bootstrap','maxnumangles','sumstuff_lookup'], compiler = mycompiler,runtime_library_dirs=["/usr/lib64/"], library_dirs=["/usr/lib64/"], libraries=["libstdc++"])
    return mysum
...
def calc_mutinf_corrected(chi_counts1, chi_counts2, bins1, bins2, chi_counts_sequential1, chi_counts_sequential2, bins1_sequential, bins2_sequential, num_sims, nbins, numangles_bootstrap, numangles, calc_variance=False,bootstrap_choose=0,permutations=0,which_runs=None,pair_runs=None, calc_mutinf_between_sims="yes", file_prefix=None, plot_2d_histograms=False):
    ...
    #print "num_pair_runs "+str(num_pair_runs)+"\n"
    ...
    #######################################################################################################
    ### Perturbation to the Kullback-Leibler Divergence, wrt. local KL-divergence of the two torsions
    ###  dKLtot/(dKL1 dKL2) = sum(bins) (1/2) * (2/pij - 1/pi - 1/pj) = (1/2) * (2*N/n_ij - N/n_i - N/n_j)
    ###
    ### for pi = n_i / N
    ###
    ###
    #dKLtot_dKL1_dKL2 = zeros((bootstrap_sets,permutations + 1),float64)
    dKLtot_dKL1_dKL2 = zeros((bootstrap_sets), float64)
    ### consider subtracting dKLtot_dKL1_dKL2 from permutations from original data
    ### because we're interested in those correlations that would be *more* than expected from independent torsions
    code = """
    for(int mybootstrap=0; mybootstrap < bootstrap_sets; mybootstrap++) {
      int mynumangles = 0;
      for(int permut=0; permut < permutations + 1; permut++) {
       for(int ibin=0; ibin < nbins; ibin++) {
        for(int jbin=0; jbin < nbins; jbin++) {
          counts_ij = *(chi_counts1_matrix + mybootstrap*bootstrap_sets*(permutations+1)*nbins*nbins
                                           + permut*nbins*nbins + ibin*nbins + jbin)
                                         
          *(dKLtot_dKL1_dKL2 + mybootstrap*bootstrap_sets*(permutations + 1)*nbins*nbins
                             + permut*nbins*nbins + ibin*nbins + jbin) +=
      ...
      """
    if(adaptive_partitioning != 0):  #this expression for dKL wouldn't be correct if we're using adaptive partitions
        for bootstrap in range(bootstrap_sets):
            nonzero_bins = logical_and((count_matrix[bootstrap,0,:] > 0), (chi_counts1_matrix[bootstrap,0,:] > 0), chi_counts2_matrix[bootstrap,0,:] > 0)
            #print "KLdiv stuff:"
            #print numangles_bootstrap_matrix
            #print numangles_bootstrap_matrix[bootstrap]
            #print (numangles_bootstrap_matrix[bootstrap,0])
            #print (numangles_bootstrap_matrix[bootstrap,0][nonzero_bins])
            # here we use nonzero_bins to "zero" the dKLtot_dKL1_dKL2 for the bins where one of the above is zero
            # we use SMALL so division doesn't blow up
            for ibin in range(nbins):
                for jbin in range(nbins):
                    if(count_matrix[bootstrap,0,ibin*nbins + jbin] >= 1 and chi_counts1[bootstrap,ibin] >= 1 and chi_counts2[bootstrap,jbin] >= 1):
                        dKLtot_dKL1_dKL2[bootstrap] += (numangles_bootstrap[bootstrap] / average(numangles_bootstrap)) * (2.0 / (count_matrix[bootstrap,0,ibin*nbins + jbin] * 1.0) - 1.0 / (chi_counts1[bootstrap,ibin] * 1.0) - 1.0 / (chi_counts2[bootstrap,jbin] * 1.0)) #NOTE divide by average(numangles_bootstrap) for numerical reasons
            #dKLtot_dKL1_dKL2[bootstrap] = sum( nonzero_bins * (numangles_bootstrap_matrix[bootstrap,0]) * ( 2.0 / ((count_matrix[bootstrap,0]) * 1.0 + SMALL) - 1.0 / ((chi_counts1_matrix[bootstrap,0]) * 1.0 + SMALL) - 1.0 / ((chi_counts2_matrix[bootstrap,0]) * 1.0 + SMALL))) #permutation zero to get rid of permutations dimension
            if(VERBOSE >= 2):
                print "nonzero bins"
                ...
                print "dKLtot_dKL1_dKL2, bootstrap:"+str(bootstrap)+" :"+str(dKLtot_dKL1_dKL2)
    #############################################################################################################
    #######################################################################################################
    ...
    if(True): #small sample stuff turned off for now because it's broken
        ...
        mutinf_thisdof = ent1_boots + ent2_boots  \
                     - sum((count_matrix * 1.0 /numangles_bootstrap_matrix)  \
                           * ( log(numangles_bootstrap_matrix) - \
    ...
    if(permutations == 0):  #then use Bayesian approach to approximate distribution of mutual information given data,prior
        ...
        Jij = (count_matrix_wprior / (numangles_bootstrap_matrix_wprior)) * logUij

        #you will see alot of "[:,0]" following. This means to take the 0th permutation, in case we're permuting data
    
        J = (sum(Jij, axis=-1))[:,0] #sum over bins ij
        K = (sum((count_matrix_wprior / (numangles_bootstrap_matrix_wprior)) * logUij * logUij, axis=-1))[:,0] #sum over bins ij
        L = (sum((count_matrix_wprior / (numangles_bootstrap_matrix_wprior)) * logUij * logUij * logUij, axis=-1))[:,0] #sum over bins ij
        ...
        #Finally, we are ready to calculate moment approximations for p(I | Data)
        E_I_mat = ((count_matrix_wprior) / (numangles_bootstrap_matrix_wprior * 1.0)) \
...
NumChis = { "ALA":0, "CYS":1, "CYN":1, "CYX":1, "ASP":2, "AS4":2, "GLU":3, "GL4": 3, "GLH":3, "PHE":2, "GLY":0, "HIS":2,
...
class ResidueChis:
   ...
   def get_num_chis(self,name):
    if NumChis.has_key(name): return min(self.max_num_chis, NumChis[name])
    else:
        mychi = 1
        numchis = 0
        #while (os.path.exists(self.xvg_basedir+"run1"+self.xvg_chidir+"chi"+str(numchis+1)+name+self.num+".xvg")):
        while (os.path.exists(self.xvg_basedir+"run1"+self.xvg_chidir+"chi"+str(numchis+1)+name+".xvg")):
      ...
      for chi_num in range(self.nchi):
         ...
         for sequential_sim_num in range(num_sims):
            ...
            if not (os.path.exists(xvg_fn) or os.path.exists(xvg_fn+".gz")):
               ...
               if myname == "HIS":
                  for myname_alt in ("HISA", "HISB", "HIE", "HID"):
                      if(self.nchi == mynumchis + 2): #phi/psi
                          if(chi_num == 0):
                              xvg_fn = basedir+"run"+str(sequential_sim_num+1)+chi_dir+"phi"+myname_alt+res_num+".xvg"
                          if(chi_num == 1):
...
##                                   searchsorted(sorted_angles,shifted_angles[chi_num,sequential_sim_num,:self.numangles[sequential_sim_num]]) # rank-ordered dihedral angles
   ...
   def __init__(self,myname,mynum,xvg_resnum,basedir,num_sims,max_angles,xvgorpdb,binwidth,sigalpha=1,
      ...
      self.nchi = self.get_num_chis(myname) * (1 - backbone_only) + phipsi
      ...
      for myscramble in range(self.permutations + 1):
          for mychi in range(self.nchi):
             ...
             if(myscramble > 0):
                 ...
                 weave.inline(code_nonadaptive_doublebins, ['num_sims', 'numangles_bootstrap', 'nbins', 'bins', 'permutations','bootstrap_sets','bootstrap_choose','angles','numangles','counts','counts2','counts3','which_runs','nchi','inv_binwidth','mychi',"max_angles",'max_num_angles','MULT_1D_BINS','FEWER_COR_BTW_BINS','SAFE_1D_BINS','TWOPI'], compiler = mycompiler,runtime_library_dirs=["/usr/lib64/"], library_dirs=["/usr/lib64/"], libraries=["libstdc++"])
                 #weave.inline(code_adaptive_doublebins, ['num_sims', 'numangles_bootstrap', 'nbins', 'bins', 'permutations','bootstrap_sets','bootstrap_choose','angles','numangles','counts_adaptive','which_runs','nchi','ent_hist_binwidths','ent_hist_left_breaks','mychi',"max_angles",'max_num_angles','MULT_1D_BINS'], compiler = mycompiler,runtime_library_dirs="/usr/lib64/", library_dirs="/usr/lib64/", libraries="libstdc++")
                 weave.inline(code_nearest_neighbor_1D, ['MAX_NEAREST_NEIGHBORS','numangles_bootstrap','permutations','bootstrap_sets','bootstrap_choose','numangles','which_runs','nchi','mychi',"max_angles",'ent_from_sum_log_nn_dists','boot_sorted_angles'], compiler = mycompiler,runtime_library_dirs=["/usr/lib64/"], library_dirs=["/usr/lib64/"], libraries=["libstdc++"])
                 ...
                 if(adaptive_partitioning == 0):
                    ...
                    weave.inline(code_nonadaptive_singlebins, ['num_sims', 'numangles_bootstrap', 'nbins', 'bins', 'permutations','bootstrap_sets','bootstrap_choose','angles','numangles','chi_pop_hist','chi_counts','which_runs','nchi','inv_binwidth','mychi',"max_angles",'max_num_angles'], compiler = mycompiler,runtime_library_dirs=["/usr/lib64/"], library_dirs=["/usr/lib64/"], libraries=["libstdc++"])
                    ...
                    weave.inline(code_nonadaptive_singlebins_sequential, ['num_sims', 'numangles_bootstrap', 'nbins_cor', 'simbins', 'permutations','bootstrap_sets','angles','numangles','chi_pop_hist_sequential','chi_counts_sequential','which_runs','nchi','inv_binwidth_cor','mychi',"max_angles",'max_num_angles','FEWER_COR_BTW_BINS'], compiler = mycompiler,runtime_library_dirs=["/usr/lib64/"], library_dirs=["/usr/lib64/"], libraries=["libstdc++"])
                 else:
                    # use bins twice as wide for mutinf between sims
                    # for bootstrap in range(bootstrap_sets):
                    #     self.numangles_bootstrap[bootstrap] = 0
                    ...
                    weave.inline(code_adaptive_singlebins, ['num_sims', 'numangles_bootstrap', 'nbins', 'bins', 'permutations','bootstrap_sets','bootstrap_choose','boot_ranked_angles','numangles','chi_pop_hist','chi_counts','which_runs','nchi','inv_binwidth_adaptive_bootstraps','mychi',"max_angles",'max_num_angles','FEWER_COR_BTW_BINS'], compiler = mycompiler,runtime_library_dirs=["/usr/lib64/"], library_dirs=["/usr/lib64/"], libraries=["libstdc++"])
                    #   for sequential_sim_num in range(num_sims):
                    #         for anglenum in range(self.numangles[sequential_sim_num]): # numangles per sim is the same regardless of chi or residue
                    ...
                    #         self.chi_pop_hist_sequential[sequential_sim_num, mychi, :] /=  self.numangles[sequential_sim_num] # normalize
                 #   weave.inline(code_nonadaptive_singlebins_sequential, ['num_sims', 'numangles_bootstrap', 'nbins_cor', 'simbins', 'permutations','bootstrap_sets','angles','numangles','chi_pop_hist_sequential','chi_counts_sequential','which_runs','nchi','inv_binwidth_cor','mychi',"max_angles",'max_num_angles'], compiler = mycompiler,runtime_library_dirs="/usr/lib64/", library_dirs="/usr/lib64/", libraries="libstdc++")
                 
                 
                    weave.inline(code_adaptive_singlebins_sequential, ['num_sims', 'numangles_bootstrap', 'nbins_cor', 'simbins', 'permutations','bootstrap_sets','rank_order_angles_sequential','chi_counts_sequential','numangles','chi_pop_hist_sequential','which_runs','nchi','inv_binwidth_adaptive_sequential','mychi',"max_angles",'max_num_angles'], compiler = mycompiler,runtime_library_dirs=["/usr/lib64/"], library_dirs=["/usr/lib64/"], libraries=["libstdc++"])
...
if __name__ == "__main__":
    ...
    mut_info_res_sumoverchis_matrix_sig_avg_01_dist = zeros(mut_info_res_matrix.shape[1:3],float32)
    mut_info_res_sumoverchis_matrix_sig_avg_05_dist = zeros(mut_info_res_matrix.shape[1:3],float32)
    ...
    for i in range(mut_info_res_matrix.shape[1]):
        for j in range(i, mut_info_res_matrix.shape[2]):
                ...
                for m in range(mut_info_res_matrix.shape[4]):
                    if(sum(mut_info_res_matrix[:,i,j,k,m]) > 0):
                        ...
                        if pval <= 0.05 and (i != j or k == m):
                            ...
                            mut_info_res_sumoverchis_matrix_sig_avg_05_dist[i,j] +=  mut_info_res_matrix_avg[i,i,k,k] + mut_info_res_matrix_avg[j,j,m,m] -2*mutinf 
                        ...
                        if pval <= 0.01 and (i != j or k == m):
                            ...
                            mut_info_res_sumoverchis_matrix_sig_avg_01_dist[i,j] +=  mut_info_res_matrix_avg[i,i,k,k] + mut_info_res_matrix_avg[j,j,m,m] -2*mutinf 
                            #if(mutinf > 0):
                                #ent1 = 0
                                #ent2 = 0
            ...
            mut_info_res_sumoverchis_matrix_sig_avg_01_dist[j,i] = mut_info_res_sumoverchis_matrix_sig_avg_01_dist[i,j]
            mut_info_res_sumoverchis_matrix_sig_avg_05_dist[j,i] = mut_info_res_sumoverchis_matrix_sig_avg_05_dist[i,j]
    ...
    if(OFF_DIAG == 1):
         ...
         output_matrix(prefix+"_bootstrap_sigavg01_mutinf_res_dist_0diag.txt",mut_info_res_sumoverchis_matrix_sig_avg_01_dist, name_num_list,name_num_list, zero_diag=True)
         ...
         output_matrix(prefix+"_bootstrap_sigavg05_mutinf_res_dist_0diag.txt",mut_info_res_sumoverchis_matrix_sig_avg_05_dist,name_num_list,name_num_list, zero_diag=True)
