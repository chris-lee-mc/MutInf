...
##                                   searchsorted(sorted_angles,shifted_angles[chi_num,sequential_sim_num,:self.numangles[sequential_sim_num]]) # rank-ordered dihedral angles
   ...
   def __init__(self,myname,mynum,xvg_resnum,basedir,num_sims,max_angles,xvgorpdb,binwidth,sigalpha=1,
      ...
      self.adaptive_hist_left_breaks = zeros((bootstrap_sets, nbins + 1),int32) #nbins plus one to define the right side of the last bin
      self.adaptive_hist_left_breaks_sequential = zeros((nbins_cor + 1 ),int32)  #nbins_cor plus one to define the right side of the last bin
      self.adaptive_hist_binwidths = zeros((bootstrap_sets, nbins ),float64) 
      self.adaptive_hist_binwidths_sequential = zeros((bootstrap_sets, nbins ),float64)
      ...
      for bootstrap in range(bootstrap_sets):
          ...
          number_per_ent_bin = (cumulative_distribution_numangles_bootstrap) / (nbins)
          ...
          for i in range(1,self.nbins):   #use cdf of weights from bootstraps
              ...
              self.adaptive_hist_left_breaks[bootstrap,i] = searchsorted( cumulative_boot_weights[bootstrap], number_per_ent_bin * i) #since searchsorted returns an array, have to get element 0 from it, corresponding to the insertion point of "number_per_ent_bin * i"
      ...
      code_cumulative_weights = """
              ...
              int angle;
              double weight;
              *(cumulative_weights + 0) = *(sorted_angles_weights + 0);
              for(int angle=1; angle < numangles_sum; angle++)
              {
                  weight = *(sorted_angles_weights + angle) ;
                  *(cumulative_weights + angle) =  *(cumulative_weights + angle - 1) + weight;
              }             
      """
      ...
      cumulative_weights = zeros((sum(self.numangles)), float64)

      weave.inline(code_cumulative_weights, ['numangles_sum', 'nbins','sorted_angles_weights','cumulative_weights'], compiler = mycompiler,runtime_library_dirs=["/usr/lib64/"], library_dirs=["/usr/lib64/"], libraries=["stdc++"])

      print shape(self.adaptive_hist_left_breaks_sequential)
      #print shape(self.adaptive_hist_binwidths)
      
      cumulative_distribution_numangles = self.numangles_bootstrap[bootstrap] #* (self.numangles_bootstrap[bootstrap] + 1) / 2  # Sum_1^n of i = n(n+1)/2
      ...
      assert(cumulative_distribution_numangles == cumulative_weights[-1])           
      number_per_ent_bin_sequential = (cumulative_distribution_numangles) / (nbins_cor)
      ...
      #rv_adaptive = stats.rv_discrete(name='adaptiveweights_sequential',values=(range(sum(self.numangles)),self.sorted_angles_weights[:sum(self.numangles)]))   
      self.adaptive_hist_left_breaks_sequential[0] = 0
      for i in range(1,nbins_cor):   #use cdf of weights from bootstraps
             self.adaptive_hist_left_breaks_sequential[i] = searchsorted(cumulative_weights, number_per_ent_bin_sequential * i) #since searchsorted returns an array, have to get element 0 from it, corresponding to the insertion point of "number_per_ent_bin_sequential * i"
      self.adaptive_hist_left_breaks_sequential[-1] = sum(self.numangles)
      ...
      print "Adaptive hist left breaks, bootstrap 0:"
      print self.adaptive_hist_left_breaks[0,:]
      print "Adaptive hist left breaks sequential, sim 0:"
      print self.adaptive_hist_left_breaks_sequential[0]
                ...
                *(numangles_bootstrap + mybootstrap) = 0 ;   
                for(int mysim=0; mysim < bootstrap_choose; mysim++) {
                    ...
                    for (int anglenum=0; anglenum< mynumangles; anglenum++) {
                      ...
                      found = 0;
                      bin_bottom = *(adaptive_hist_left_breaks + mybootstrap*nbins 
                                     + 0);
                      for(int bin1=0; bin1 < nbins; bin1++) {
                          //printf("bin: %i",bin1);
                          bin_top = *(adaptive_hist_left_breaks + mybootstrap*nbins 
                                     + bin1 +1 );
                          if(found == 0 && angle >= bin_bottom && angle < bin_top) {
                ...
                for(bin1 = 0; bin1 < nbins; bin1++) {
                    ...
                    *(chi_pop_hist + mybootstrap*nchi*nbins + mychi*nbins + bin1)
                    ...
                    //printf("%f , %f \\n",*(chi_counts + mybootstrap*nchi*nbins + mychi*nbins + bin1),binwidth);
                }
              }
              """
              ...
              double weight, bin_bottom, bin_top;
                for (int simnum=0; simnum < num_sims; simnum++) {
                    ...
                    for (int anglenum=0; anglenum< mynumangles; anglenum++) {
                      angle = *(rank_order_angles_sequential + mychi*num_sims*max_angles  + simnum*max_angles + anglenum);
                      weight = *(weights + simnum*max_angles + anglenum); //assumes mynumangles same for all sims
                      //if(simnum == 0) printf("angle:%i, weight:%f \\n",angle, weight);
                      found = 0;
                      bin_bottom = *(adaptive_hist_left_breaks_sequential + 
                                     + 0);
                      for(int bin1=0; bin1 < nbins_cor; bin1++) {
                          //printf("bin: %i",bin1);
                          bin_top = *(adaptive_hist_left_breaks_sequential + 
                                     + bin1 + 1);
                          if(found == 0 && angle >= bin_bottom && angle < bin_top) {
